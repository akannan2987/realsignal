...
**Against the authors' own code.** Their original R scripts ship with
the data, so we can run them. This matters more than it sounds. If our
numbers and the paper's disagree, a two-way comparison cannot tell you
*why*: our rebuild might be wrong, or their released code might not
reproduce their own paper, or the finding might be sensitive to the
toolchain. *Analogy:* a magazine recipe gives you a flat cake — your
baking, a misprint, or your oven? With only the magazine and the cake
you cannot say; watching the original chef bake it settles it at once.
Running their code is watching the chef. The three-way comparison
(paper → their R → our Python) isolates each possibility.
...
|---|---|---|
| **Streamlit** | The analyst — a quick internal tool | Phase 7 |
| **React + TypeScript** | The end user — the product | Phase 11 |
| **MCP server** | An AI assistant | Phase 15 |
...
limits. Once the API from Phase 10 exists, this is a thin, well-designed
...
├── api/           ← the FastAPI backend                  (Phase 10)
├── app/           ← the Streamlit prototype              (Phase 7)
├── frontend/      ← the React + TypeScript product       (Phase 11)
├── airflow/       ← the orchestration DAGs               (Phase 12)
├── mcp/           ← the MCP server                       (Phase 15)
...
| **R + RStudio** | A language built by statisticians, for statistics, with an editor made for it | The original analysis is written in R; running it is what makes the three-way comparison possible | Reading the R only, without running it — you would be guessing at what it does |
